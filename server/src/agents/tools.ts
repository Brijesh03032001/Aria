...
// ── Web Search — Tavily (free) or Perplexity (paid) ─────────
// Priority: TAVILY_API_KEY → PERPLEXITY_API_KEY → disabled (returns [])
// Tavily: https://app.tavily.com — free 1000 searches/month, no credit card
// Perplexity: https://www.perplexity.ai/settings/api — paid only
...
async function searchWithTavily(query: string): Promise<string> {
  ...
  const answer = data.answer ?? data.results.map(r => r.content).join("\n\n");
  const citations = data.results.map(r => r.url);

  return JSON.stringify({ success: true, answer, citations });
}

async function searchWithPerplexity(query: string, recency: string): Promise<string> {
  const res = await fetch("https://api.perplexity.ai/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are a search assistant. Provide accurate, concise answers with specific details like prices, ratings, and product names. Focus on the most relevant results.",
        },
 ...
 * Uses Tavily if TAVILY_API_KEY is set (free 1k/month).
 * Falls back to Perplexity if PERPLEXITY_API_KEY is set (paid).
 * Returns empty array if neither key is set.
 */
export function createSearchTools(): DynamicStructuredTool[] {
  const provider = TAVILY_API_KEY ? "tavily" : PERPLEXITY_API_KEY ? "perplexity" : null;
  if (!provider) return [];

  console.log(`[web_search] Using provider: ${provider}`);

  const webSearch = new DynamicStructuredTool({
    name: "web_search",
    description:
      "Search the web for real-time information. Returns a grounded answer with source citations. Use this INSTEAD of navigating to Google. Best for: product comparisons, prices, reviews, news, factual questions, 'best X for Y' queries.",
    schema: z.object({
      query: z
        .string()
    ...
    func: async (input: any) => {
      try {
        ...
        console.log(`[web_search] provider=${provider} query="${input.query}"`);

        const result = provider === "tavily"
          ? await searchWithTavily(input.query)
          : await searchWithPerplexity(input.query, input.search_recency ?? "week");

        console.log(`[web_search] done in ${Date.now() - startTime}ms`);
        return result;
      } catch (err) {
