...
export async function initElasticsearch(): Promise<boolean> {
  ...
  try {
    const nodeUrl = process.env.ELASTIC_URL;
    client = nodeUrl
      ? new Client({ node: nodeUrl, auth: { apiKey }, Connection: HttpConnection })
      : new Client({ cloud: { id: cloudId }, auth: { apiKey }, Connection: HttpConnection });
